    ...
    if (process.env.NODE_ENV === 'development') {
      const auditSEO = () => {
        ...
        if (!title || title.textContent!.length < 30 || title.textContent!.length > 60) {
          issues.push('Title tag missing or not optimized (30-60 characters recommended)');
        }
        
        // Check for meta description
        const metaDesc = document.querySelector('meta[name="description"]') as HTMLMetaElement;
        if (!metaDesc || metaDesc.content.length < 120 || metaDesc.content.length > 160) {
          issues.push('Meta description missing or not optimized (120-160 characters recommended)');
        }
        ...
        if (!ogTitle || !ogDesc || !ogImage) {
          issues.push('Open Graph tags incomplete (title, description, image required)');
        }
        ...
        const internalLinks = document.querySelectorAll('a[href^="/"], a[href*="ashentrix.com"]');
